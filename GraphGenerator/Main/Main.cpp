...
namespace {
   ...
   os << R"(USAGE: ArchitectureMining [OPTION] <directory> <ignoredFilePaths> <ignoredNamespaces> <ST-output>

OPTIONS:

  --src <directory>
        Mine the whole directory containing the sources (not recommended) .

  --cmp-db <directory>
        Use the compilation database generated by cmake ( ex. compile_commands.json) .

ARGUMENTS:

  <ignoredFilePaths>
        Path to the file containing ignored file paths.

  <ignoredNamespaces>
        Path to the file containing ignored namespaces.

  <ST-output>
        Path to the output file for ST (whatever ST is).

NOTE:
  <ignored*> files are NL separated items:
      ex. for ignoredNamespaces.txt
        `
                std
                __gnu_cxx
                _IO_FILE
